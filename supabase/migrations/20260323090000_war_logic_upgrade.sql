-- War logic upgrade: eligibility filters, auto-start/auto-finish helpers, and secure
-- question access for official war participants using unapproved quizzes.
...
-- 3) Validate joins for wars (capacity, rating, unrated policy, late-join cutoff).
...
BEGIN
  ...
  IF v_match.is_official THEN
    ...
    END IF;
  ELSE
    -- Friendly behavior: keep waiting-only joins.
...
-- 4) Public helper to progress due wars (called from clients periodically).
...
BEGIN
  -- Auto-start official wars at their declared time.
  ...
  -- Auto-finish official wars once duration is over.
...
-- 5) Let all authenticated users view participants of official wars.
...
-- 6) Allow official-war participants to read unapproved war questions only after war starts.
...
-- 7) Helpful indexes for war filtering.
